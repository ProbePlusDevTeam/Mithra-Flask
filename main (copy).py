...
UPLOAD_FOLDER = "/home/karthi/Projects/Mithra-Flask/static/Uploads"
DOWNLOAD_FOLDER = "/home/karthi/Projects/Mithra-Flask/static/Downloads"
REPORT_FOLDER = "/home/karthi/Projects/Mithra-Flask/static/Reports"
SYNC_FOLDER = "/home/karthi/Projects/Mithra-Flask/static/Sync_Datas"
FailedAPI = "/home/karthi/Projects/Mithra-Flask/static/Failed_Offline"
Tokens = "/home/karthi/Projects/Mithra-Flask/static/Token"
...
def logout():
    ...
    time_data = {"user_pri_id": session['user_id'], "user_name": session['user'], "date_time": str(
        datetime.now()), "role": session['role'], "device_id": "Web", "action1": "logout"}
    time_role = "admin"
    time_method = "POST"
    time_responses = {}
...
def login_check():
    ...
    if request.method == "POST":
        ...
        data = {"user_name": user_name,
                "password": password, "device_id": "web"}
        role = "admin"
        method = "POST"
        responses = {}
        responses = apicall(method, url, data, role)
        ...
        if "message" in responses:
            ...
            if responses == "Password is incorrect, kindly reenter the password":
                ...
                time_url = "api/method/mithra.mithra.doctype.login_time.api.add_login_time"
                time_data = {"user_pri_id": user_id, "user_name": user_name, "date_time": str(
                    datetime.now()), "role": role, "device_id": "Web", "action1": "login"}
                time_role = "admin"
                time_method = "POST"
                time_responses = {}
                time_responses = apicall(
                    time_method, time_url, time_data, time_role)
        ...
        if g.user:
            if request.method == "POST":
                ...
                if responses == "available":
                    ...
                    data = {"user_name": email_id, "password": password, "role": "study_investigator", "active": "yes",
                            "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                    role = "admin"
                    method = "POST"
                    responses = {}
                    responses = apicall(method, url, data, role)
                    ...
                    if email_id == responses["message"][0]["user_name"]:
                        ...
                        data = {"user_pri_id": responses["message"][0]['user_pri_id'], "first_name": first_name, "last_name": last_name, "age": age, "gender": gender,
                                "mobile_number": mobile_number, "email_id": email_id, "active": "yes", "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                        role = "admin"
                        method = "POST"
                        responses = {}
                        responses = apicall(method, url, data, role)
        ...
        if g.user:
            if request.method == "PUT":
                ...
                if responses == "available":
                    ...
                    method = "PUT"
                    responses = {}
                    data = {"pri_id": PkId, "first_name": first_name, "last_name": last_name, "age": age, "gender": gender,
                            "mobile_number": mobile_number, "email_id": email_id, "active": active, "modified_user": session['user_id']}
                    responses = apicall(method, url, data, role)
                    responses = responses["message"]
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                if responses == "available":
                    url = "api/method/mithra.mithra.doctype.user_table.login.useradd"
                    data = {"user_name": email_id, "password": password, "role": "coordinator", "active": "yes",
                            "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                    role = "admin"
                    method = "POST"
                    responses = {}
                    ...
                    if email_id == responses[0]["user_name"]:
                        ...
                        data = {"user_pri_id": responses[0]['user_pri_id'], "first_name": first_name, "last_name": last_name, "age": age, "gender": gender, "mobile_number": mobile_number,
                                "email_id": email_id, "active": "yes", "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                        role = "admin"
                        method = "POST"
                        responses = {}
                        responses = apicall(method, url, data, role)
        ...
        if g.user:
            if request.method == "PUT":
                ...
                if responses == "available":
                    ...
                    method = "PUT"
                    responses = {}
                    data = {"pri_id": pri_id, "first_name": first_name, "last_name": last_name, "age": age, "gender": gender,
                            "mobile_number": mobile_number, "email_id": email_id, "active": active, "modified_user": session['user_id']}
                    responses = apicall(method, url, data, role)
                    responses = responses["message"]
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                if responses == "available":
                    url = "api/method/mithra.mithra.doctype.user_table.login.useradd"
                    data = {"user_name": email_id, "password": password, "role": "researcher", "active": "yes",
                            "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                    role = "admin"
                    method = "POST"
                    responses = {}
        ...
        if g.user:
            if request.method == "PUT":
                ...
                data = {"full_name": full_name, "age": age, "gender": gender,
                        "mobile_number": mobile_number, "email_id": email_id, "modified_user": session['user_id']}
                responses = apicall(method, url, data, role)
        ...
        if g.user:
            ...
            if "diabetes_mellitus" in disease_responses:
                if disease_responses["diabetes_mellitus"] != "no":
                    ...
                    disease_result["diabetes_mellitus"] = {
                        "diagnosed": res[0], "age": res[1], "treatment": res[2], "activities": res[3]}
            ...
            if "chronic_liver_disease" in disease_responses:
                if disease_responses["chronic_liver_disease"] != "no":
                    ...
                    texts = texts.replace("]", "")
                    res = list(texts.split(","))
                    disease_result["chronic_liver_disease"] = {
                        "diagnosed": res[0], "age": res[1], "treatment": res[2], "activities": res[3]}
            ...
            if "chronic_renal_disease" in disease_responses:
                if disease_responses["chronic_renal_disease"] != "no":
                    ...
                    texts = texts.replace("]", "")
                    res = list(texts.split(","))
                    disease_result["chronic_renal_disease"] = {
                        "diagnosed": res[0], "age": res[1], "treatment": res[2], "activities": res[3]}
            ...
            if "gastric_problem" in disease_responses:
                if disease_responses["gastric_problem"] != "no":
                    ...
                    texts = texts.replace("]", "")
                    res = list(texts.split(","))
                    disease_result["gastric_problem"] = {
                        "diagnosed": res[0], "age": res[1], "treatment": res[2], "activities": res[3]}
            ...
            if "other_diseases" in disease_responses:
                if disease_responses["other_diseases"] != "no":
                    ...
                    texts = texts.replace("]", "")
                    res = list(texts.split(","))
                    disease_result["other_diseases"] = {
                        "diagnosed": res[0], "age": res[1], "treatment": res[2], "activities": res[3], "specification": res[4]}
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                data = {"loc_id": loc_id, "panchayat": panchayat, "village": village, "shg": shg, "coordinator": coordinator, "researcher": researcher,
                        "eligible": eligible, "group1": group1, "active": "yes", "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                role = "admin"
                method = "POST"
                responses = apicall(method, url, data, role)
                result = {}
    ...
    try:
        if g.user:
            if request.method == "PUT":
                ...
                url = url
                data = {"name": PkId, "loc_id": loc_id, "panchayat": panchayat, "village": village, "shg": shg, "eligible": eligible,
                        "group1": group1, "coordinator": coordinator, "researcher": researcher, "active": active, "modified_user": session['user_id']}
                role = "admin"
                method = "PUT"
                responses = {}
                responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                data = {"sub_module_no": sub_module_no, "sur_pri_id": sur_id, "video_name_e": video_name_e, "video_name_k": video_name_k, "video_location_e": video_location_e,
                        "video_location_k": video_location_k, "active": "yes", "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                role = "admin"
                method = "POST"
                responses = {}
                responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                eng_file_exist = os.path.exists(os.path.join(
                    app.config['UPLOAD_FOLDER'] + "/Videos/", video_e_filename.filename))
                kan_file_exist = os.path.exists(os.path.join(
                    app.config['UPLOAD_FOLDER'] + "/Videos/", video_k_filename.filename))
                ...
                if eng_file_exist:
                    ...
                    if kan_file_exist:
                        ...
                        video_k_filename.save(os.path.join(
                            app.config['UPLOAD_FOLDER'] + "/Videos/", video_k_filename.filename))
                        video_e_filename.save(os.path.join(
                            app.config['UPLOAD_FOLDER'] + "/Videos/", video_e_filename.filename))

                        url = "api/method/mithra.mithra.doctype.video.api.add_video"
                        data = {"sub_module_no": sub_module_no, "sur_pri_id": sur_id, "video_name_e": video_name_e, "video_name_k": video_name_k, "video_location_e": video_e_filename.filename,
                                "video_location_k": video_k_filename.filename, "active": "yes", "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                        role = "admin"
                        method = "POST"
                        responses = {}
                        responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == "PUT":
                ...
                data = {"name": PkId, "sub_module_no": sub_module_no, "sur_pri_id": sur_id, "video_name_e": video_name_e, "video_name_k": video_name_k,
                        "active": active, "video_location_e": location_e, "video_location_k": location_k, "modified_user": session['user_id']}
                role = "admin"
                method = "PUT"
                responses = {}
                responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                data = {"device_id": device_id, "device_details": device_details, "device_type": device_type, "allotment_cor": alloted_coordinator,
                        "allotment_res": alloted_researcher, "active": "yes", "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                role = "admin"
                method = "POST"
                responses = apicall(method, url, data, role)
                data = {}
    ...
    try:
        if g.user:
            if request.method == "PUT":
                ...
                data = {"name": PkId, "device_id""": device_id, "device_details": device_details, "device_type": device_type,
                        "allotment_cor": alloted_coordinator, "allotment_res": alloted_researcher, "active": active, "modified_user": session['user_id']}
                role = "admin"
                method = "PUT"
                responses = {}
                responses = apicall(method, url, data, role)
                ...
                for que in q_responses:
                    if version == "eng":
                        ...
                        for opt in op_responses:
                            ...
                            opt_list["opt" +
                                     str(i) + "_subbranch"] = opt["option_sb"]
                            opt_list["opt" + str(i) +
                                     "_weight"] = opt["option_w"]
                            i = i + 1
                        opt_list["no_opt"] = i - 1
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                data = {"sur_pri_id": survey_id, "section": section, "section_name": section_name_e, "section_name_k": section_name_k, "qn_number": que_number, "question_e": que_e,
                        "question_k": que_k, "qn_branch": que_branch, "option_type": opttype, "active": "yes", "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                role = "admin"
                method = "POST"
                responses = apicall(method, url, data, role)
                result = {}
                    ...
                    if opttype == "single_select" or opttype == "multi_select":
                        try:
                            for x in range(num):
                                ...
                                data = {"sur_pri_id": survey_id, "question_id": queid, "option_no": int(
                                    x + 1), "option_e": opt_e[x], "option_k": opt_k[x], "option_w": option_w[x], "option_sb": opt_sb[x], "active": "yes", "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                                role = "admin"
                                method = "POST"
                                responses = apicall(method, url, data, role)
                        except:
    ...
    try:
        if g.user:
            if request.method == "PUT":
                ...
                data = {"name": PkId, "sur_pri_id": survey_id, "section": section, "section_name": section_name_e, "section_name_k": section_name_k, "qn_number": que_number,
                        "question_e": que_e, "question_k": que_k, "qn_branch": que_branch, "option_type": opttype, "active": q_active, "modified_user": session['user_id']}
                role = "admin"
                method = "PUT"
                responses = apicall(method, url, data, role)
                result = {}
                    ...
                    try:
                        for x in range(len(opt_e)):
                            if opt_id[x] != "null":
                                ...
                                data = {"name": opt_id[x], "option_no": opt_no[x], "sur_pri_id": survey_id, "question_id": queid, "option_e": opt_e[x],
                                        "option_k": opt_k[x], "option_w": opt_w[x], "option_sb": opt_sb[x], "active": q_active, "modified_user": session['user_id']}
                                role = "admin"
                                method = "PUT"
                                responses = apicall(method, url, data, role)
                            else:
                                ...
                                url = "api/method/mithra.mithra.doctype.survey_question_options.api.add_survey_option"
                                data = {"sur_pri_id": survey_id, "question_id": queid, "options": option, "active": "yes",
                                        "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                                role = "admin"
                                method = "POST"
                                responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == 'POST':
                ...
                eng_file_exist = os.path.exists(os.path.join(
                    app.config['UPLOAD_FOLDER'] + "/Audios/", engfile.filename))
                kan_file_exist = os.path.exists(os.path.join(
                    app.config['UPLOAD_FOLDER'] + "/Audios/", kanfile.filename))
                ...
                if eng_file_exist:
                    ...
                    if kan_file_exist:
                        ...
                        engfile.save(os.path.join(
                            app.config['UPLOAD_FOLDER'] + "/Audios/", engfile.filename))
                        kanfile.save(os.path.join(
                            app.config['UPLOAD_FOLDER'] + "/Audios/", kanfile.filename))

                        url = "api/resource/survey_questions/"
                        url = url + ques_audio_id
                        data = {"audio_e": engfile.filename, "audio_k": kanfile.filename,
                                "modified_user": session['user_id']}
                        role = "admin"
                        method = "PUT"
                        responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                data = {"survey_name": survey_name, "active": "yes",
                        "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                role = "admin"
                method = "POST"
                responses = apicall(method, url, data, role)
        ...
        if g.user:
            if request.method == "POST":
                ...
                for data in data1:
                    ...
                    value = {"survey_pri_id": PkId, "repeat": data, "active": "yes",
                             "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                    data = value
                    role = "admin"
                    method = "POST"
    ...
    try:
        if g.user:
            if request.method == "PUT":
                ...
                data = {"name": survey_pri_id, "active": "yes",
                        "survey_name": survey_name, "modified_user": session['user_id']}
                role = "admin"
                method = "PUT"
                responses = apicall(method, url, data, role)
                responses = responses["message"]
                ...
                if responses == "success":
                    ...
                    data = {"name": PkId, "survey_pri_id": survey_pri_id, "days": days, "plusdays": plusdays, "minusdays": minusdays,
                            "group1": group1, "filled_by": filled_by, "active": active, "modified_user": session['user_id']}
                    role = "admin"
                    method = "PUT"
                    responses = apicall(method, url, data, role)
                    responses = responses["message"]
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                data = {"survey_pri_id": survey_pri_id, "module_number": module_number, "sub_module_name": sub_module_name, "sub_module_number": sub_module_number,
                        "group1": group1, "active": "yes",  "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                role = "admin"
                method = "POST"
                responses = {}
                responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == "PUT":
                ...
                data = {"module_id": PkId, "survey_pri_id": survey_pri_id, "module_number": module_number, "sub_module_name": sub_module_name,
                        "sub_module_number": sub_module_number, "group1": group1, "active": active, "modified_user": session['user_id']}
                role = "admin"
                method = "PUT"
                responses = {}
                responses = apicall(method, url, data, role)
        ...
        if g.user:
            if request.method == "POST":
                ...
                if cfdata["interval1"] != "null":
                    ...
                    interval1 = "0"
                module_data = {"week_no": week_no,
                               "module_no": module_no, "interval1": interval1}
                module_data = str(module_data)

                url = "api/method/mithra.mithra.doctype.module_mapping.api.add_module_mapping"
                data = {"survey_pri_id": survey_pri_id, "description": description, "min_score": min_score, "max_score": max_score, "survey_instance_days": str(
                    survey_instance_days), "module_data": module_data, "active": "yes",  "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                role = "admin"
                method = "POST"
                responses = {}
                responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == "PUT":
                ...
                data = {"name": PkId, "min_score": min_score, "max_score": max_score, "week_no": week_no, "module_no": module_no, "interval1": interval1, "survey_instance_days":
                        survey_instance_days, "active": active, "module_data": module_data, "description": description, "survey_pri_id": sur_id, "modified_user": session['user_id']}
                role = "admin"
                method = "PUT"
                responses = {}
                responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == "POST":
                ...
                data = {"sur_pri_id": sur_pri_id, "activity_name_e": activity_name_e, "activity_name_k": activity_name_k, "sub_module_number": sub_module_number, "duration": duration, "time_spent": time_spent, "mood": mood,
                        "intensity_e": intensity_e, "intensity_k": intensity_k, "active": "yes", "image_location": image_location, "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                role = "admin"
                method = "POST"
                responses = {}
                responses = apicall(method, url, data, role)
    ...
    try:
        if g.user:
            if request.method == "PUT":
                ...
                data = {"name": PkId, "sur_pri_id": sur_pri_id, "activity_name_e": activity_name_e, "activity_name_k": activity_name_k, "sub_module_number": sub_module_number,
                        "duration": duration, "time_spent": time_spent, "mood": mood, "intensity_e": intensity_e, "intensity_k": intensity_k, "active": active, "modified_user": session['user_id']}
                role = "admin"
                method = "PUT"
                responses = {}
                responses = apicall(method, url, data, role)
            ...
            if request.method == "POST":
                ...
                try:
                    ...
                    file_exist = os.path.exists(os.path.join(
                        app.config['UPLOAD_FOLDER'] + "/Images/", imgfile.filename))
                    ...
                    if file_exist:
                        ...
                        imgfile.save(os.path.join(
                            app.config['UPLOAD_FOLDER'] + "/Images/", imgfile.filename))
                        url = "api/method/mithra.mithra.doctype.activity.api.add_activity"
                        data = {"sur_pri_id": sur_pri_id, "activity_name_e": activity_name_e, "activity_name_k": activity_name_k, "sub_module_number": sub_module_number, "duration": duration, "time_spent": time_spent, "mood": mood,
                                "intensity_e": intensity_e, "intensity_k": intensity_k, "active": "yes", "image_location": imgfile.filename, "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
                        role = "admin"
                        method = "POST"
                        responses = {}
                        responses = apicall(method, url, data, role)
...
def addreason():
    try:
        if g.user:
            ...
            url = "api/resource/reason"
            data = {"reason": reason, "reason_category": reason_category, "active": "yes",
                    "created_user": session['user_id'], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified"}
            role = "admin"
            method = "POST"
            responses = {}
...
def editreason(PkId):
    try:
        if g.user:
            ...
            data = {"name": PkId, "reason": reason, "reason_category": reason_category,
                    "active": "yes", "modified_user": session['user_id']}
            role = "admin"
            method = "PUT"
            responses = {}
            responses = apicall(method, url, data, role)
    ...
    try:
        if request.method == "POST":
            ...
            if foldername == "Image":
                ...
                file_exist = os.path.exists(os.path.join(
                    app.config['UPLOAD_FOLDER'] + "/Images/", image.filename))
                if file_exist:
                    ...
                    image.save(os.path.join(
                        app.config['UPLOAD_FOLDER'] + "/Images/", image.filename))
                    data = {"image_location": image.filename,
                            "modified_user": session['user_id']}
                    role = "admin"
                    method = "PUT"
                    responses = {}
            ...
            if foldername == "Audio":
                ...
                eng_file_exist = os.path.exists(os.path.join(
                    app.config['UPLOAD_FOLDER'] + "/Audios/", eng_audio.filename))
                kan_file_exist = os.path.exists(os.path.join(
                    app.config['UPLOAD_FOLDER'] + "/Audios/", kan_audio.filename))
                ...
                if eng_file_exist:
                    ...
                    if kan_file_exist:
                        ...
                        kan_audio.save(os.path.join(
                            app.config['UPLOAD_FOLDER'] + "/Audios/", kan_audio.filename))
                        eng_audio.save(os.path.join(
                            app.config['UPLOAD_FOLDER'] + "/Audios/", eng_audio.filename))
                        url = "api/resource/survey_questions/"
                        url = url + PkId
                        data = {"audio_e": eng_audio.filename,
                                "audio_k": kan_audio.filename, "modified_user": session['user_id']}
                        role = "admin"
                        method = "PUT"
                        responses = {}
                        responses = apicall(method, url, data, role)
            ...
            if foldername == "Video":
                ...
                eng_file_exist = os.path.exists(os.path.join(
                    app.config['UPLOAD_FOLDER'] + "/Videos/", eng_video.filename))
                kan_file_exist = os.path.exists(os.path.join(
                    app.config['UPLOAD_FOLDER'] + "/Videos/", kan_video.filename))
                ...
                if eng_file_exist:
                    ...
                    if kan_file_exist:
                        ...
                        kan_video.save(os.path.join(
                            app.config['UPLOAD_FOLDER'] + "/Videos/", kan_video.filename))
                        eng_video.save(os.path.join(
                            app.config['UPLOAD_FOLDER'] + "/Videos/", eng_video.filename))

                        url = "api/resource/video/"
                        url = url + PkId
                        data = {"video_location_e": eng_video.filename,
                                "video_location_k": kan_video.filename, "modified_user": session['user_id']}
                        role = "admin"
                        method = "PUT"
                        responses = {}
                        responses = apicall(method, url, data, role)
    ...
    try:
        ...
        for key in key_list:
            ...
            for api in full_value:
                ...
                act_responses = {}
                act_responses = apicall(
                    act_method, act_url, act_data, act_role)
                if api["table"] == "par_video_data":
                    if api["method"] == "POST":
                        store_ids[act_data["name"]
                                  ] = act_responses["message"]["name"]
                if "message" in act_responses:
                    if act_responses["message"] == "success" or act_responses["message"]["status"] == "success":
                        ...
                        sync_method = "POST"
                        sync_data = {"api_device_pri_id": api["api_pri_key"], "method": api["method"], "url": api["url"], "body": api["body"], "table_name": api["table"], "device_id": api[
                            "device_id"], "created_user": api["coordinator_id"], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified", "status": "pass",  "error_log": "no error log"}
                        now = datetime.now()
                        now = str(now)
                        sync_responses = apicall(
                            sync_method, sync_url, sync_data, sync_role)
                else:
                    ...
                    if "exception" in act_responses:
                        sync_data = {"api_device_pri_id": api["api_pri_key"], "method": api["method"], "url": api["url"], "body": api["body"], "table_name": api["table"], "device_id": api["device_id"],
                                     "created_user": api["coordinator_id"], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified", "status": "fail", "error_log": act_responses["exception"]}
                        sync_responses = apicall(
                            sync_method, sync_url, sync_data, sync_role)

                    else:
                        sync_responses = {"api_device_pri_id": api["api_pri_key"], "method": api["method"], "url": api["url"], "body": api["body"], "table_name": api["table"], "device_id": api[
                            "device_id"], "created_user": api["coordinator_id"], "modified_user": "UT-9-2022-07-11-08:19:58-no_one_modified", "status": "fail", "error_log": act_responses}
                        sync_responses = apicall(
                            sync_method, sync_url, sync_data, sync_role)
